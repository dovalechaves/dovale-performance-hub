/**
 * Adiciona UMA coluna nova ("Média de Venda 2026 (Qtde)") ao arquivo que o Willian já
 * editou (título "FERRAGENS PA" + cabeçalho na linha 2 + dados a partir da linha 3),
 * sem tocar em mais nada do arquivo — usa exceljs (não xlsx) pra preservar a
 * formatação que ele já aplicou, em vez de reconstruir a planilha do zero.
 *
 * Média = soma da quantidade vendida no ano de 2026 (todas as bases somadas) dividida
 * pelo número de meses que tiveram venda (não pelos meses corridos do ano) — confirmado
 * com o Willian.
 *
 * Bases (7, as pedidas): SJC, MG (= "SPM" no painel de comissão), LOCKEY MG,
 * LOCKEY SP + FAST (mesma base, emp_fil_codigo 11/12), LOCKEY RS (MySQL), NITERÓI
 * (MySQL), EP (SQL Server) — mesmas conexões/queries já usadas em
 * scripts/relatorio-produtos-media-mensal-areas-2026.ts e no painel de comissão
 * (server/services/comissao/dados-externos.ts).
 *
 * Filtro de venda (bases Firebird): pdv_psi_codigo NOT IN ('CC'),
 * pdv_tve_codigo NOT IN ('6','7','26','34'), exclui clientes internos
 * ('44274','98030','49268') — padrão do projeto.
 *
 * Rodar: npx tsx scripts/ferragens-pa-media-venda-2026.ts
 */
...
async function coletarFirebird(
  ...
  for (const lote of chunk(codigos, 400)) {
    ...
    const sqlText = `
      select p.pro_codigo as codigo, extract(month from ped.pdv_data) as mes, sum(i.pvi_quantidade) as qtde
    ...
    for (const r of rows) {
      out.push({ codigo: String(r.CODIGO).trim(), mes: Number(r.MES), qtde: Number(r.QTDE) || 0 });
    }
  }
  ...
  try {
    for (const lote of chunk(codigos, 400)) {
      ...
      const [rows] = await conn.query(
        `
        select p.CodigoPro as codigo, month(o.\`Data\`) as mes, sum(i.Qtd) as qtde
...
async function coletarEP(codigos: string[]): Promise<Ponto[]> {
  ...
  for (const lote of chunk(codigos, 400)) {
    ...
    const r = await pool
      ...
      .query(`
        select p.CODIGO as codigo, month(e.[DATA]) as mes, sum(p.QTD) as qtde
        from EP e
    ...
    for (const row of r.recordset as any[]) {
      out.push({ codigo: String(row.codigo).trim(), mes: Number(row.mes), qtde: Number(row.qtde) || 0 });
    }
  }
...
async function main() {
  console.log("=== FerragensPA: adicionando coluna 'Média de Venda 2026 (Qtde)' ===");
  ...
  for (const p of todosPontos) {
    ...
    m.set(p.mes, (m.get(p.mes) || 0) + p.qtde);
  }
  ...
  for (const codigo of codigos) {
    ...
    let soma = 0;
    let mesesComVenda = 0;
    for (const qtde of meses.values()) {
      soma += qtde;
      if (qtde > 0) mesesComVenda++;
    }
  ...
  headerRow.getCell(novaColIdx).value = "Média de Venda 2026 (Qtde)";
