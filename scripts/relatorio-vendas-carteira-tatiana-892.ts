/**
 * Relatório: Valor de venda mês a mês do representante 892 (Tatiana Rocha),
 * bases SJC e MG, janeiro/2025 a agosto/2026.
 *
 * Importante: a venda é atribuída pela CARTEIRA do cliente (clientes.cli_rep_codigo = 892),
 * não pelo representante lançado no pedido (pedidos_vendas.pdv_rep_codigo) — ou seja,
 * entra toda venda feita para um cliente que tem a Tatiana como representante no
 ...
 *
 * Rodar: npx tsx scripts/relatorio-vendas-carteira-tatiana-892.ts
 */
...
const REP_CODIGO = 892;
const REP_NOME_ESPERADO = "FERRAGENS TATIANA ROCHA";
...
function gerarChavesMeses(): { chave: string; label: string; ano: number; mes: number }[] {
  ...
  for (let ano = 2025; ano <= 2026; ano++) {
    const mesInicio = ano === 2025 ? 1 : 1;
    const mesFim = ano === 2026 ? 8 : 12;
...
async function main() {
  ...
  for (const base of BASES) {
    ...
    if (!repNome.toUpperCase().includes("TATIANA")) {
