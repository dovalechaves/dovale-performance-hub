...
router.get("/vendedor/:nome", async (req: any, res: any) => {
  ...
  try {
    ...
    // Meta mensal, bônus e percentual de recorrência configurado — 3 queries
    // independentes entre si, buscadas em paralelo (cada uma preserva seu
    // próprio fallback/catch, igual ao que faziam antes em sequência).
    const [metaVendedor, bonusResult, recorrenciaPercentualConfigurado] = await Promise.all([
      mes
        ? pool.request()
      ...
      is_televendas
        ...
        : Promise.resolve({ recordset: [] as Array<Record<string, unknown>> }),
      is_televendas
        ? getRecorrenciaPercentualConfigurado(pool)
        : Promise.resolve(RECORRENCIA_PERCENTUAL),
    ]);

    let bonusConfig: BonusConfig | null = bonusResult.recordset.length
