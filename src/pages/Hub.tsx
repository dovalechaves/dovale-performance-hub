...
import { BarChart3, Calculator, LogOut, Sun, Moon, Users, RefreshCw, Loader2, ChevronDown, Settings2, Send, Archive, Bot, Database, ClipboardList, UserPlus, ShieldCheck, BellRing } from "lucide-react";
            ...
            <div className="rounded-xl border border-border overflow-x-auto">
              <table className="w-full text-sm min-w-[1900px]">
                ...
                <tbody>
                  {usersLoading ? (
                    <tr>
                      <td colSpan={27} className="px-4 py-8 text-center text-muted-foreground">
                        <Loader2 className="w-5 h-5 animate-spin mx-auto" />
                      </td>
                    </tr>
                  ) : filteredManagedUsers.length === 0 ? (
                    <tr>
                      <td colSpan={27} className="px-4 py-8 text-center text-muted-foreground text-xs">
                        {appUserFilter === "all"
                    ...
                    pagedUsers.map((u) => (
                      <tr key={u.usuario} className="border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors">
                            ...
                            onChange={async (e) => {
                              ...
                              const next: AuthManagedUser = {
                                ...
                                apps: {
                                  ...
                                  score: {
                                    ...u.apps.score,
                                    can_access: enabled ? u.apps.score.can_access : false,
                                  },
                                  cobranca: {
                                    ...u.apps.cobranca,
                        ...
                        <td className="px-4 py-3 text-center">
                          <input
                            type="checkbox"
                            checked={u.apps.inventario.can_access}
                            onChange={async (e) => {
                              const next: AuthManagedUser = {
                                ...u,
                                apps: {
                        ...
                        <td className="px-4 py-3">
                          <div className="relative inline-block">
                            <select
                              value={u.apps.inventario.usu_codigo_sistema ?? ""}
                              onChange={async (e) => {
                              ...
                              disabled={savingUser === u.usuario || !u.can_access_hub || !u.apps.inventario.can_access}
                              className="appearance-none rounded-lg border border-border bg-muted px-3 py-1.5 pr-7 text-xs text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:opacity-40"
                            >
                              <option value="">—</option>
                              {fbUsers.map((fb) => (
                                <option key={fb.codigo} value={fb.codigo}>{fb.codigo} - {fb.nome}</option>
                              ))}
                            </select>
                            <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground pointer-events-none" />
                          </div>
                        ...
                        <td className="px-4 py-3 text-center">
                          <input
                            type="checkbox"
                            checked={u.apps.score.can_access}
                            onChange={async (e) => {
                              const next: AuthManagedUser = {
                                ...u,
                                apps: {
                        ...
                        <td className="px-4 py-3 text-center">
                          <input
                            type="checkbox"
                            checked={u.apps.cobranca.can_access}
                            onChange={async (e) => {
                              const next: AuthManagedUser = {
                                ...u,
                                apps: {
