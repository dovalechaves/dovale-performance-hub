...
import {
  ...
  ChevronDown, ChevronRight, RotateCcw, FileText, Clock, Filter, MapPin, Download,
} from "lucide-react";
...
export default function Inventario() {
  ...
  // ── State ──
  const [view, setView] = useState<View>(searchParams.has("sessao") ? "detail" : "list");
  ...
  useEffect(() => {
    if (locais.length === 1) setContagemLocalId(locais[0].id);
  }, [locais]);
  ...
  useEffect(() => {
    const sid = searchParams.get("sessao");
    if (sid) loadDetail(Number(sid));
  ...
  useEffect(() => {
    ...
    const socket: Socket = ioClient(socketUrl, { transports: ["websocket", "polling"] });
  ...
  const openDetail = (s: Sessao) => {
    ...
    setItemSearch("");
    setItemPage(1);
    setSearchParams({ sessao: String(s.id) });
    loadDetail(s.id);
  };
  ...
  // ── Export report (.xlsx with 3 sheets) ──
  const exportReport = () => {
    if (!selectedSessao) return;

    const header = ["Código", "Descrição", "Qtd Sistema", ...locais.map((l) => l.nome), "Total Contado", "Diferença", "Custo Fiscal", "Vlr Diferença"];

    const buildRows = (items: Item[]) =>
      items.map((item) => {
        const localQtds = locais.map((l) => {
          const c = item.contagens?.find((c) => c.local_id === l.id);
          return c?.qtd_contada ?? null;
        });
        const diff = item.qtd_contada !== null ? item.qtd_contada - item.qtd_sistema : null;
        const vlrDiff = diff !== null && item.custo_fiscal !== null ? diff * item.custo_fiscal : null;
        return [
          item.pro_codigo,
          item.descricao ?? "",
          item.qtd_sistema,
    ...
    const makeSheet = (items: Item[]) => {
      const rows = buildRows(items);
      return XLSX.utils.aoa_to_sheet([header, ...rows]);
    };

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, makeSheet(itens), "Todos");
    XLSX.utils.book_append_sheet(wb, makeSheet(itens.filter((i) => i.qtd_contada !== null)), "Contados");
    XLSX.utils.book_append_sheet(wb, makeSheet(itens.filter((i) => i.qtd_contada === null)), "Não Contados");

    XLSX.writeFile(wb, `inventario_${selectedSessao.id}_${selectedSessao.nome.replace(/\s+/g, "_")}.xlsx`);
  ...
  const filteredItems = useMemo(() => {
    let list = itens;
    // Viewer: only items they personally counted
    if (isViewer) {
      list = list.filter((i) =>
        i.contagens?.some((c) => c.contado_por === usuario && c.qtd_contada !== null)
      );
    }
      ...
      <header className="border-b border-border bg-gradient-card">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => view === "detail" ? backToList() : navigate("/hub")}
          ...
          <div className="space-y-6">
            ...
            {!detailLoading && (
              <>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {isManager && (
                      <button onClick={exportReport}
                        className="inline-flex items-center gap-1 rounded-lg bg-secondary px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-muted transition-colors">
                ...
                {isViewer ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl border border-green-500/30 bg-green-500/5 p-4 text-center">
                      ...
                      <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Meus Itens Contados</p>
                    </div>
                  </div>
                ) : (
                ...
                {canInsertContagem && (
                  <div
                    className="rounded-xl border border-teal-500/30 bg-teal-500/5 p-4 cursor-pointer hover:bg-teal-500/10 transition-colors"
                    onClick={() => setShowContagem((v) => !v)}
                  >
